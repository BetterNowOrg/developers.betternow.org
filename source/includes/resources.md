...
Following is a list of the resources available via the API, the operations that
are supported for each resource, and examples of response data.

The `curl` examples use the `-n` flag to pull information from the `.netrc`
file. For more information see the [curl example above](/#curl-example).
